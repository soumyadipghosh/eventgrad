...
int main(int argc, char *argv[])
{
  ...
  //create memory window for parameters from left and right (STILL CASTING AS FLOAT TYPE)
  ...
  MPI_Win_create(win_mem, win_size*param_elem_size, param_elem_size, MPI_INFO_NULL, MPI_COMM_WORLD, &win);
  ...
  //Number of epochs where event condition not verified (due to starting oscillations)
  ...
  for (size_t epoch = 1; epoch <= 250; ++epoch)
  {
	  ...
	  loss.backward();
          ...
          for(auto i = 0; i < sz; i++)
          {
             ...
             if(std::fabs(curr_norm - last_norm[i]) >= findThreshold(epoch, constant, gamma)
                                                       || epoch < initial_comm_epochs)
             {
                ...
                //PUSH ENTIRE MSG
                //send to left
                MPI_Win_lock(MPI_LOCK_EXCLUSIVE, left, 0, win); //changed from SHARED
                MPI_Put(temp, flat.numel(), MPI_FLOAT, left, (num_elem_param + disp), flat.numel(), MPI_FLOAT, win);
                //MPI_Win_flush(left, win);
                MPI_Win_unlock(left, win);
                
                //send to right
                MPI_Win_lock(MPI_LOCK_EXCLUSIVE, right, 0, win);
                MPI_Put(temp, flat.numel(), MPI_FLOAT, right, disp, flat.numel(), MPI_FLOAT, win);
                ...
                for(int j = 0; j < flat.numel(); j++)
                {
                   ...
                   MPI_Put((temp + j), 1, MPI_FLOAT, left, (num_elem_param + disp + j), 1, MPI_FLOAT, win);
                   MPI_Win_flush(left, win);
                   MPI_Win_unlock(left, win);

                   //send to right
                   MPI_Win_lock(MPI_LOCK_EXCLUSIVE, right, 0, win);
                   MPI_Put((temp + j), 1, MPI_FLOAT, right, (disp + j), 1, MPI_FLOAT, win);
                   MPI_Win_flush(right, win);
                   MPI_Win_unlock(right, win);
                }
                */
             ...
             //unpack 1-D vector from corresponding displacement and form tensor
   ...
   std::cout << "No of events in rank " << rank << " - " << num_events << std::endl;
   ...
   MPI_Allreduce(MPI_IN_PLACE, &num_events, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
   if(rank == 0) std::cout << "Total number of events - " << num_events << std::endl;
