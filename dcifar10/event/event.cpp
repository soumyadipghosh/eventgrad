...
int main(int argc, char *argv[]) {
  ...
  ResNet<BasicBlock> model(layers, 10); //ResNet-18
  ...
  // Number of epochs
  auto num_epochs = options.epochs;
    ...
    for (auto &batch : *train_loader) {
      ...
      if (numranks > 1) {
        for (auto i = 0; i < sz; i++) {
          ...
          float *temp = (float *)calloc(num, param_elem_size);
          std::memcpy(temp, flat.data_ptr(), num * param_elem_size);
          ...
          if (value_diff >= *(thres + i) || pass_num < initial_comm_passes) {
            ...
            MPI_Put(temp, num, MPI_FLOAT, left, (num_elem_param + disp), num,
                    MPI_FLOAT, win);
            ...
            MPI_Put(temp, num, MPI_FLOAT, right, disp, num, MPI_FLOAT, win);
          ...
          // Left neighbor
          float *left_recv = (float *)calloc(num, param_elem_size);
          float left_temp = 0.0;
          std::memcpy(left_recv, (win_mem + disp), num * param_elem_size);

          // forming left tensor
          torch::Tensor left_tensor =
              torch::from_blob(left_recv, dim_array, torch::kFloat).clone();
          ...
          */
          ...
          // Right Neighbor
          float *right_recv = (float *)calloc(num, param_elem_size);
          float right_temp = 0.0;
          std::memcpy(right_recv, (win_mem + num_elem_param + disp),
                      num * param_elem_size);

          // forming right tensor
          torch::Tensor right_tensor =
              torch::from_blob(right_recv, dim_array, torch::kFloat).clone();
